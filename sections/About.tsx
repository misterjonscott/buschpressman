...
const About = () => {
  ...
  return (
    <div 
      ref={sectionRef}
      id="about" 
      className="section-container relative min-h-screen flex items-center justify-center flex-col py-12 md:py-20 z-0" 
    >
      {/* Hero Layout */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          {features.map((feature, index) => {
            ...
            return (
              <div 
                key={index}
                className="relative flex flex-col items-start text-left p-6 md:p-8 rounded-2xl bg-card/30 border border-border/40 hover:bg-card/60 hover:border-border/80 transition-all duration-300 w-full group shadow-sm"
              >
                {/* Inline Header Row Lockup */}
                <div className="flex items-center gap-3 mb-3 w-full">
                  {/* Micro-glow Icon Container aligned tight with title */}
